...
**Status (2026-07).** Everything below is stable and validated. The belief trunk is
**goal-displacement** (Sally-Anne — solved in every run, and the family the behavioural
probe discriminates on) together with the **two-observer** family that generalises it to
two agents in one world; the single-agent wall, witness and false-obstacle families
harden them, and all recover the shared-`av` agency constructor. The
**`dual_belief`** family (two agents holding *contradictory* false beliefs) has been
**deleted**: it named six latent literals and two nested forks, solved 0/24 in every run
including the long-timeout ones, and the two-observer family makes the same argument at
a fraction of the search cost — see Sections 4.2 and 5.
...
- **Root type / interpreter dispatch.** A program's *root type* selects how it is
  ...
  root types.
- **Matching.** A task is a **set of `k` scenes sharing one latent program**
  (`scenes.py`; `K_SCENES = 4`, CLI `--k`), so a program is a solution iff, for
  *every* scene, running it from that scene's frame-0 grid for that scene's own `T`
  reproduces the exact trajectory (`scenes.solves`; `mat_key` hashes shape + bytes
  over all scenes). On a hit the solution is `simplify`-ed and stored; ties are
  broken by `length` (shorter wins).

  One trajectory does not determine its program — the corpus is finite and the DSL
  is not — so a single-scene task is solvable for the wrong reason, and the
  generators used to answer each such coincidence with another rejection filter.
  Requiring all `k` scenes is the general form of that check. It is close to free:
  `solves` short-circuits on the first scene that disagrees, and essentially every
  enumerated program dies on scene 1, so the average cost per program is ~1 unfold
  per task — the same as the single-scene matcher. Only a program that *already*
  reproduced scene 1 pays for the remaining `k-1`, which is exactly the coincidence
  worth testing. Downstream nothing changes: one task still yields one solution, so
  the joint stitch sees the same programs over the same library.
...
Tasks are `(Scenes, metadata)` pairs — `Scenes` being a set of `k` trajectories
generated by one latent program from different initial grids (§2.3). Each family is
generated *through the real interpreter*, and the `k`-scene format is what makes the
intended program the unique cheapest explanation: what is held fixed across a task's
scenes is exactly the literals its program names (the agent/goal values, the phantom
wall's coordinate), so a rival has to fit all of them. The exhaustive catalogue — every
family's ground-truth program, latent, and count — is in
[`PRIMITIVES_AND_TASKS.md`](PRIMITIVES_AND_TASKS.md); the summary here.
...
- **`fn`-rooted** (`unfold`, a `grid→grid` transition): `physics`, `desire`, `belief`
  (five generators — `make_goal_displacement_tasks` / `make_two_observer_tasks` /
...
beliefs — was **never solved in any run** and has been deleted in favour of
`belief_observers`; Section 5.)
...
| `belief_dual` (two contradictory beliefs) | **Deleted** — 0 solves in every run incl. long-timeout; not a budget miss (six latent literals and two nested forks may not be enumerable in this DSL). Superseded by `belief_observers`, which makes the same "no single-grid program explains this" argument for one direction literal instead of two coordinate pairs, and generates 24/24 tasks in 0.6 s. |
| `belief_observers` (two observers, one world) | **New (2026-07)** — two agents, one goal, one grid: `ao` seeks the true goal bare, `av` seeks a privately displaced one. Generates readily (24/24 tasks, ~0.6 s) and asks the sharper question: does the library apply the invented constructor *selectively*, to `av` and not to `ao`? |
