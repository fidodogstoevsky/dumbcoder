...
== Rerunning the repaired corpus <sec-rerun>

The results reported in @ch-results comes from a pair of runs on one version of the corpus, and since those runs were made two of the generators have been repaired, in both cases in the direction of making the scenes harder to explain without a belief. Neither repair has been run, so the first thing to do is the experiment already described, on the corpus as it now stands.

The witness family's phantom wall cost the believer nothing. The family exists to defeat the transient-wall rival with a second agent that walks through the cell the first agent avoids (@fig-belief-witness), and that argument turns on the witness's crossing; but the generator's check that the believer also _pays_ for its belief was vacuous, and on the corpus the runs used, 185 of 192 scenes have the believer on a shortest path --- the believed wall changed which shortest path the agent took and nothing else. The scenes do not literally leak: the obvious rival, two plain seekers positing no wall at all, reproduces 0 of the 48 tasks, so nothing in the reported rival exclusions is false as stated. But it fails only on `optimize`'s arbitrary tie-break among equally short paths, and in 42 of the 48 tasks a seeker with some other tie-break would reproduce the believer exactly --- a far weaker exclusion than the one the family was built to give, which is that no real wall could have permitted what the frames show. The repair, applied but unrun, requires the believer to spend strictly more steps than its Manhattan distance to the goal and rejects outright any scene set the wall-free rival reproduces; valid scenes become about sixteen times rarer, but the family refills at its old size, 48 of 48 tasks with 0 free-detour scenes. Every witness number in @ch-results is therefore a number about a corpus that no longer exists. The prediction on rerunning is that the solutions look as they did and the exclusion becomes structural rather than tie-break-dependent; the risk is that harder scenes push the family out of the per-task budget, and that too would be worth knowing.

The Sally--Anne family carried a defence that had become a liability. Each goal-displacement scene contained an inert bystander object, kept so that a "beacon" rival --- a seek aimed at some third value that happens to sit on the agent's route --- would have to find its target in four different places. Post @corpus that argument is circular, because the bystander is what supplies the beacon with a target: swept over the closed set of beacon spellings, the best reproduces 1 of 4 scenes with a bystander and 0 of 4 without. Removing it also returns search budget, since the prior of @enumeration makes every visible value a free literal at each of the belief program's four value slots: in smoke tests a one-shove task went unsolved with seven free values, solved in 77 s with three and in 24 s with two. The prediction is that the largest single block of missing coverage --- 21 of 48 goal-displacement tasks (@ch-results) --- shrinks, which doubles as the cheap partial test of the budget-miss claim described in @sec-budget: a change that only makes the family cheaper to _find_, leaving what has to be found untouched, should not move coverage at all if the missing tasks are out of reach rather than out of time.

Those two families are the whole of the difference: hashing the generated tasks before and after the repairs shows 24 of the 25 families byte-for-byte identical. The episode's general lesson is a preference the corpus already leans on. An exclusion built into the _scene_ --- the bystander standing on the phantom cell, the witness crossing it, the goal that never moves --- holds under any pricing and any tie-break, whereas an exclusion that has to be enumerated holds only over the list someone wrote down. The rerun should be read as moving the witness family from the second kind of defence to the first.
